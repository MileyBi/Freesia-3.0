...
ImportDialog::ImportDialog(){
    ...
    for(int i=0;i<3;i++){
        ...
        p->setRange(0,500);p->setValue(1);voxelLyt->addWidget(p);
    }
    Common *c=Common::i();QStringList voxelValues=c->readConfig("import_voxels").split(" ");
    if(voxelValues.length()==3){for(int i=0;i<3;i++){m_voxelBoxes[i]->setValue(voxelValues[i].toFloat());}}
...
ImportDialog::~ImportDialog(){
    QStringList voxelValues;for(int i=0;i<3;i++){voxelValues.append(m_voxelBoxes[i]->text());}
...
bool ImportDialog::getParameters(ImportParams &p){
    ...
    for(int i=0;i<3;i++){p.voxels[i]=m_voxelBoxes[i]->value();}
